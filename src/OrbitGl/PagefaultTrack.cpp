...
PagefaultTrack::PagefaultTrack(CaptureViewElement* parent, TimeGraph* time_graph,
    ...
    : Track(parent, time_graph, viewport, layout, capture_data, indentation_level),
      ...
      minor_pagefault_track_{std::make_shared<BasicPagefaultTrack>(this, time_graph, viewport,
                                                                   layout, "Minor Pagefault Track",
                                                                   series_names, capture_data)} {
  const std::string kPagefaultTrackName = "Pagefault Track";
  SetName(kPagefaultTrackName);
  SetLabel(kPagefaultTrackName);
}
...
float PagefaultTrack::GetHeight() const {
  ...
  if (!major_pagefault_track_->IsEmpty()) {
    height += major_pagefault_track_->GetHeight() + layout_->GetSpaceBetweenGpuSubtracks();
  }
  if (!minor_pagefault_track_->IsEmpty()) {
    height += minor_pagefault_track_->GetHeight() + layout_->GetSpaceBetweenGpuSubtracks();
  }
  return height;
}
...
void PagefaultTrack::UpdatePositionOfSubtracks() {
  ...
  if (!minor_pagefault_track_->IsEmpty()) {
    current_y -= (layout_->GetSpaceBetweenGpuSubtracks() + major_pagefault_track_->GetHeight());
  }
